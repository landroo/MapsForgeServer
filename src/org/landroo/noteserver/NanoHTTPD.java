 ...
 * 
 * <p>
 * KtoHTTPD version 0.1 (based on NanoHTTPD 1.14), Copyright &copy;
 * 2001,2005-2010 Jarno Elonen (elonen@iki.fi, http://iki.fi/elonen/) and
 * Copyright &copy; 2010 Konstantinos Togias (info@ktogias.gr,
 * http://ktogias.gr)
 * 
 * <p>
...
public class NanoHTTPD
{
	 ...
	 */
	public Response serve(String uri, String method, Properties header, Properties parms, Properties files, String post)
	{
...
/*	public static void main(String[] args)
	...
	private class HTTPSession implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				// We write the part of body already read to
				// ByteArrayOutputStream f
				ByteArrayOutputStream f = new ByteArrayOutputStream();
				if(rlen > splitbyte) f.write(buf, splitbyte, rlen - splitbyte);
				//else NoteServer.Log.i("NanoHTTPD", "" + buf.length + " " + rlen + " " + splitbyte);
					
				// While firefox sends on the first read all the data fitting
				// our buffer, chrome and opera sends only the headers even if
				// there is data for the body. So we do some magic here to find
				// out whether we have already consumed part of body, if we have
				// reached the end of the data to be sent or we should expect
				// the first byte of the body at the next read.
				...
				// We also create a BufferedReader for easily reading it as
				// string.
				...
				if(uri != null)
				{
					Response r = serve(uri, method, header, parms, files, postLine);
		...
		private void decodeHeader(BufferedReader in, Properties pre, Properties parms, Properties header)
		{
			...
			catch (Exception ex)
			{
				try
				{
					sendError(HTTP_INTERNALERROR, "SERVER INTERNAL ERROR: Exception: " + ex);
				}
				catch (Throwable t)
				{
				}
		...
		/**
		 * Retrieves the content of a sent file and saves it to a temporery
	...
	public Response serveFile(String uri, Properties header, File homeDir, boolean allowDirectoryListing)
	{
		...
		if (f.isDirectory())
		{
			...
			{
				uri += "/";
				Response r = new Response(HTTP_REDIRECT, MIME_HTML, "<html><body>Redirected: <a href=\"" + uri + "\">" + uri
						+ "</a></body></html>");
			...
			else if (allowDirectoryListing)
			{
				...
				for (int i = 0; i < files.length; ++i)
				{
					...
					if (curFile.isFile())
					{
						...
						else if (len < 1024 * 1024) msg += curFile.length() / 1024 + "." + (curFile.length() % 1024 / 10 % 100)
								+ " KB";
						else msg += curFile.length() / (1024 * 1024) + "." + curFile.length() % (1024 * 1024) / 10 % 100
								+ " MB";
	...
	static
	{
		StringTokenizer st = new StringTokenizer("css            text/css " + "js             text/javascript "
				+ "htm		text/html " + "html		text/html " + "txt		text/plain " + "asc		text/plain " + "gif		image/gif "
				+ "jpg		image/jpeg " + "jpeg		image/jpeg " + "png		image/png " + "mp3		audio/mpeg " + "m3u		audio/mpeg-url "
				+ "pdf		application/pdf " + "doc		application/msword " + "ogg		application/x-ogg "
				+ "zip		application/octet-stream " + "exe		application/octet-stream " + "class		application/octet-stream ");
		while (st.hasMoreTokens())
